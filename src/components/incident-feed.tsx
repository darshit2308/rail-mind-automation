...
const KIND_STYLES: Record<FeedKind, { border: string; label: string; text: string }> = {
  ...
  dispatch: { border: "border-l-agent", label: "DISPATCH", text: "text-agent" },
  step: { border: "border-l-primary", label: "ACTION", text: "text-primary" },
      ...
      {active.length > 0 && (
        <div className="space-y-1.5 border-b border-border p-2.5">
          {active.map((i) => (
            ...
            >
              <span className={`rounded px-1.5 py-0.5 font-mono text-[9px] font-bold ${SEV_STYLES[i.severity]}`}>
